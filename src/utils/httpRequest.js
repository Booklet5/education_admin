...
let localFileUrl = 'https://education-dev-1253719016.cos.ap-nanjing.myqcloud.com'

http.getFileHost = function () {
  return (process.env.NODE_ENV !== 'production' && process.env.OPEN_PROXY ? localFileUrl : window.SITE_CONFIG.fileUrl)
}

const webSocketHost = 'localhost'

http.getWebSocketHost = function () {
  return (process.env.NODE_ENV !== 'production' && process.env.OPEN_PROXY ? webSocketHost : window.SITE_CONFIG.webSocket)
}
export default http
