  ...
  export default {
    ...
    methods: {
      ...
      changePermission (roleId) { // 设置权限
        let that = this
        that.roleId = roleId
        that.axios.get(that.$http.httpUrl('/system/menu/selectMenuListByRoleId'), {
          params: {
            roleId: that.roleId
          }
      ...
      savePermission () {
        ...
        that.axios.post(that.$http.httpUrl('/system/role/saveRolePermission'), params).then(function (response) {
      ...
      getTreeMenuList () {
        let that = this
        that.axios.get(that.$http.httpUrl('/system/menu/menuTreeList'))
          .then(function (response) {
